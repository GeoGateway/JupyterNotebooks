...
def create_grid(array,spacing=.005):
    '''
...
def reshape_and_create_df(grid_x, grid_y, z1, new =True, z_name ='Z'):
    ...
    if new==True:
        ...
        df_dict = {'Lon': array[0,range(size),0], 'Lat': array[0,range(size),1],\
...
def interpolate(x, y,  grid_spacing = .004, model='spherical',returngrid = False,**kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if counter==0:
            OK = OrdinaryKriging(x, y, v, variogram_model=model, verbose = False,\
                            enable_plotting=False, nlags = 40, coordinates_type='geographic')
            ...
            OK = OrdinaryKriging(x, y, v, variogram_model=model, verbose = False,\
                            enable_plotting=False, nlags = 40)
            z1, ss1 = OK.execute('grid', grid_x,grid_y,mask = False)
            vals=np.ma.getdata(z1)
            sigma = np.ma.getdata(ss1)
